...
class ArticleController extends AuthController
{
    ...
    public function actions()
    {
        return [
            ...
            'getlist' => [
                'class' => 'backend\actions\GetNewsListAction'
            ]
        ];
    }
    ...
    {
        $url = Url::toRoute(["article/getlist"]);
        $initJs = [
    ...
    public function actionUpdate(){
        ...
        if (Yii::$app->request->isPost){
            ...
            if(isset($param['news_id']) && $param['news_id']){
                $model = ZfbNews::findOne($param['news_id']);
                $model->uid = 1;
                ...
                $model->news_title = trim($param['news_title']);
                $model->news_author = trim($param['news_author']);
                $model->meta_keyword = trim($param['meta_keyword']);
                $model->meta_desp = trim($param['meta_desp']);
                $model->news_content = trim($param['news_content']);
