 ...
 */
class GetCustBankListAction extends Action {
    public function run() {
        $request = Yii::$app->request;
        $param = $request->post();

        $row = ZfbCustBank::find()->with('cust')->leftJoin('zfb_cust', 'zfb_cust.cust_id = zfb_cust_bank.cust_id');
        ...
        if(isset($param['start_date']) && $param['start_date'] && isset($param['end_date']) && $param['end_date']){
            ...
            $row->andFilterWhere(['between','zfb_cust_bank.ctime',$start_date, $end_date]);
    ...
    private function _loadData($data){
        ...
        foreach($data  as $key=>$item){
            ...
            $datas[]=array(
                '<th width="2%"><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"><input type="checkbox" class="group-checkable" name="id" value="'.$item['bank_id'].'" /><span></span></label></th>',
                $item['cust']['cust_name'],
