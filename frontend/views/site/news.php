        ...
        <div class="row">
            ...
            <div class="main col-md-8 col-md-offset-1">
                ...
                <?php foreach($noticeList as $val){
                    $url = Url::toRoute(['site/article', 'id' => $val['news_id']]);
                    ?>
                    <!-- notice start -->
                    <article class="clearfix blogpost object-non-visible" data-animation-effect="fadeInUpSmall" data-effect-delay="200">
                        ...
                        <div class="blogpost-body">
                            ...
                            <div class="blogpost-content" style="height:165px; overflow:hidden; margin-bottom:10px;">
                                <header>
                                    <h2 class="title"><a href="<?=$url?>"><?=$val['news_title']?></a></h2>
                                    <div class="submitted"><i class="fa fa-user pr-5"></i> by <a href="<?=$url?>"><?=$val['news_author']?></a></div>
                                </header>
                                <?=$val['news_content']?>
                            </div>
                        </div>
                ...
                <!-- pagination start -->
                <ul class="pagination">
                    <li><a href="<?=Url::toRoute(['site/article', 's' => 0])?>">«</a></li>
                    <?php $page = ceil($recordTotal/5);
                    for($i=0; $i< $page; $i++){?>
                        <li <?php if($s/5 == $i){?>class="active"<?php } ?>><a href="<?=Url::toRoute(['site/news', 's' => $i*5])?>"><?=($i+1)?>
                                <?php if($s/5 == $i){?><span class="sr-only">(current)</span><?php } ?></a>
                        </li>
                    <?php } ?>
                    <li><a href="<?=Url::toRoute(['site/article', 's' => $page])?>">»</a></li>
                </ul>
                <!-- pagination end -->
